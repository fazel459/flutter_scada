...
class _ReportScreenState extends ConsumerState<ReportScreen> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() { _dataLoading = true; _error = null; });
  ...
  void _setQuickRange(String range) {
    ...
    switch (range) {
      case '1h': _fromDate = now.subtract(const Duration(hours: 1)); break;
      case '6h': _fromDate = now.subtract(const Duration(hours: 6)); break;
      case '24h': _fromDate = now.subtract(const Duration(hours: 24)); break;
      case '7d': _fromDate = now.subtract(const Duration(days: 7)); break;
      case '30d': _fromDate = now.subtract(const Duration(days: 30)); break;
      case '90d': _fromDate = now.subtract(const Duration(days: 90)); break;
    }
    _toDate = now;
    _quickRange = range;
    setState(() {});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('📊 Reports', style: TextStyle(color: Colors.white, fontSize: 18)),
        actions: [
          IconButton(icon: const Icon(Icons.file_download, color: Colors.white70), onPressed: _exportCsv, tooltip: 'Export CSV'),
          IconButton(icon: const Icon(Icons.refresh, color: Colors.white70), onPressed: _loadData, tooltip: 'Refresh'),
        ],
      ),
      body: Row(
        children: [
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Text('Select Tags', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 13)),
                const Spacer(),
                  ...
                  child: SizedBox(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFF475569))),
                  ...
                  onPressed: () {
                    setState(() {
                      if (_selectedTagIds.length == _allTags.length) {
                        ...
                        _selectedTagIds = _allTags.map((t) => t['widgetId'] as String).toSet();
                      }
                    });
                  },
                  child: Text(_selectedTagIds.length == _allTags.length ? 'None' : 'All',
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return CheckboxListTile(
                            ...
                            onChanged: (v) {
                              setState(() {
                                if (v == true) _selectedTagIds.add(id);
                                else _selectedTagIds.remove(id);
                              });
                            },
                            title: Text(tag['widgetLabel'] ?? 'Unknown',
                                style: TextStyle(color: selected ? Colors.blue : Colors.white70, fontSize: 12)),
                            subtitle: tag['pageTitle'] != null
                                ? Text(tag['pageTitle'] as String, style: const TextStyle(color: Color(0xFF64748B), fontSize: 9))
                                : null,
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Text('Time Range', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 12)),
                  ],
                ),
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: active ? Colors.blue.withOpacity(0.3) : Colors.black.withOpacity(0.3),
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(color: active ? Colors.blue : const Color(0xFF475569)),
                        ),
                        child: Text(r, style: TextStyle(color: active ? Colors.blue : const Color(0xFF94A3B8), fontSize: 10)),
                      ),
                    );
                  }).toList(),
                ),
                ...
                Row(
                  children: [
                    Expanded(
                      child: _dateButton('From', _fromDate, (d) => setState(() { _fromDate = d; _quickRange = ''; })),
                    ),
                    const SizedBox(width: 6),
                    Expanded(
                      child: _dateButton('To', _toDate, (d) => setState(() { _toDate = d; _quickRange = ''; })),
                    ),
                  ],
                ),
                ...
                Row(
                  children: [
                    const Text('Aggregate:', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 10)),
                    const SizedBox(width: 6),
                    Expanded(
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          onChanged: (v) => setState(() => _aggregateMode = v ?? 'none'),
                        ),
                      ),
                    ),
                  ],
                ...
                if (_aggregateMode != 'none') ...[
                  ...
                  Row(
                    children: [
                      const Text('Interval:', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 10)),
                      const SizedBox(width: 6),
                      Expanded(
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<int>(
                            ...
                            onChanged: (v) => setState(() => _intervalMinutes = v ?? 5),
                          ),
                        ),
                      ),
                    ],
          ...
          Container(
            ...
            child: ElevatedButton.icon(
              onPressed: _selectedTagIds.isEmpty || _dataLoading ? null : _loadData,
              icon: _dataLoading
  ...
  Widget _dateButton(String label, DateTime date, Function(DateTime) onChanged) {
    return GestureDetector(
      onTap: () async {
                ...
                : _reportData == null
                    ? const Center(
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.analytics_outlined, color: Colors.white24, size: 64),
                            SizedBox(height: 12),
                    ...
                    : _dataLoading
                        ? const Center(child: CircularProgressIndicator())
                        : _viewMode == 'stats' ? _buildStatsView()
                        : _viewMode == 'chart' ? _buildChartView()
                        : _buildTableView(),
          ),
        ],
      ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text('$icon $label',
            style: TextStyle(color: active ? Colors.blue : const Color(0xFF94A3B8), fontSize: 11)),
      ),
    );
  }
  ...
  Widget _buildTableView() {
    ...
    if (series.isEmpty) return const Center(child: Text('No data', style: TextStyle(color: Colors.white38)));
    ...
    allRows.sort((a, b) {
      int cmp;
      if (_sortColumn == 'tag') cmp = (a['tag'] as String).compareTo(b['tag'] as String);
      else if (_sortColumn == 'value') cmp = ((a['value'] as double) - (b['value'] as double)).sign.toInt();
      else cmp = (a['timestamp'] as DateTime).compareTo(b['timestamp'] as DateTime);
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 50, child: Text('Unit', style: TextStyle(color: Color(0xFF94A3B8), fontSize: 10, fontWeight: FontWeight.w600))),
              ],
            ),
          ),
            ...
            child: Row(
              children: [
                SizedBox(
                  ...
                  child: Text(DateFormat('MM/dd HH:mm:ss').format(row['timestamp'] as DateTime),
                      style: TextStyle(color: Colors.white.withOpacity(0.6), fontSize: 11, fontFamily: 'monospace')),
                ),
                SizedBox(
                  width: (MediaQuery.of(context).size.width - 300) * 0.375,
                  child: Text(row['tag'] as String, style: const TextStyle(color: Colors.white, fontSize: 12)),
                ),
                SizedBox(
                  width: (MediaQuery.of(context).size.width - 300) * 0.25,
                  child: Text(
                    (row['value'] as double).toStringAsFixed(2),
                    style: const TextStyle(color: Colors.blue, fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'monospace'),
                  ),
                ),
                SizedBox(
                  width: 50,
                  child: Text(row['unit'] as String, style: const TextStyle(color: Color(0xFF94A3B8), fontSize: 10)),
                ),
              ],
            ),
          )),
    ...
    return Expanded(
      ...
      child: GestureDetector(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(label, style: TextStyle(color: active ? Colors.blue : const Color(0xFF94A3B8), fontSize: 10, fontWeight: FontWeight.w600)),
            if (active) Icon(_sortAsc ? Icons.arrow_upward : Icons.arrow_downward, size: 12, color: Colors.blue),
          ],
        ),
      ),
    );
            ...
            children: series.map((s) {
              ...
              return Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text('${s['widgetLabel']} (${s['unit']})', style: const TextStyle(color: Colors.white70, fontSize: 10)),
                ],
              );
            }).toList(),
          ),
  ...
  Color _chartColor(int index) {
    const colors = [Color(0xFF3B82F6), Color(0xFF22C55E), Color(0xFFEF4444), Color(0xFFEAB308),
        ...
        return Card(
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('🏷️ ${s['widgetLabel']}', style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold)),
                const Divider(color: Color(0xFF334155)),
                _statRow('Count', '${points.length}'),
                _statRow('Average', '${avg.toStringAsFixed(2)} ${s['unit'] ?? ''}'),
                _statRow('Minimum', '${min.toStringAsFixed(2)} ${s['unit'] ?? ''}'),
                _statRow('Maximum', '${max.toStringAsFixed(2)} ${s['unit'] ?? ''}'),
                _statRow('Sum', '${total.toStringAsFixed(2)} ${s['unit'] ?? ''}'),
...
class _ReportChartPainter extends CustomPainter {
  ...
  static const _colors = [Color(0xFF3B82F6), Color(0xFF22C55E), Color(0xFFEF4444),
    Color(0xFFEAB308), Color(0xFF8B5CF6), Color(0xFF06B6D4)];
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final padding = EdgeInsets.fromLTRB(60, 10, 20, 40);
    ...
    for (var i = 0; i < series.length; i++) {
      ...
      for (final p in pts) {
        ...
        final x = padding.left + chartW * (t.difference(from).inMilliseconds / totalMs).clamp(0.0, 1.0);
        final y = padding.top + chartH * (1 - ((v - globalMin) / range).clamp(0.0, 1.0));
    ...
    for (int i = 0; i <= 5; i++) {
      ...
      labelPainter.text = TextSpan(text: val.toStringAsFixed(1), style: const TextStyle(color: Color(0xFF64748B), fontSize: 8));
      labelPainter.layout();
      final y = padding.top + chartH * (1 - i / 5);
      labelPainter.paint(canvas, Offset(padding.left - labelPainter.width - 6, y - labelPainter.height / 2));
    }

    // Time labels
    final df = DateFormat('MM/dd HH:mm');
    labelPainter.text = TextSpan(text: df.format(from), style: const TextStyle(color: Color(0xFF64748B), fontSize: 8));
    labelPainter.layout();
    labelPainter.paint(canvas, Offset(padding.left, size.height - 28));
    labelPainter.text = TextSpan(text: df.format(to), style: const TextStyle(color: Color(0xFF64748B), fontSize: 8));
    labelPainter.layout();
    labelPainter.paint(canvas, Offset(size.width - padding.right - labelPainter.width, size.height - 28));
  }
